...
config = dotenv_values("/ceph/kkorini/LLM/TabAnnGPT/key.env")
    ...
    for j, table_preds in enumerate(preds):
        ...
        if len(col_preds) < table_number-1:
            for m in range(0, table_number-1-len(col_preds)):
                ...
                num +=1
                i+=1
    return predictions, num, table_indices
    ...
    for j, table_preds in enumerate(preds):
        ...
        if len(col_preds) < table_number:
            for m in range(0, table_number-len(col_preds)):
                ...
                num +=1
                i+=1
    return predictions, num, oov_indices, oov, oov_table_indices, oov_tablecolumn_indices, table_indices, column_indices
